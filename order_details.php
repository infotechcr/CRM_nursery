<?php include_once 'header.php'; include_once 'query.php'; 
	...
	if(isset($_GET['u_bill_no']))
  {
    ...
    $_SESSION['billno'] = $billno;
  	$select_cat_wise_order = "select category.* , product_order.* , user.* FROM product_order join category on category.cat_id=product_order.cat_id join user on user.u_id=product_order.user_id where product_order.bill_no=$billno GROUP BY product_order.cat_id";
  ...
  if(isset($_GET['o_id']))
  {
      ...
      $update_order_status = "update product_order set packing_status=1 where o_id=$o_id";
      mysqli_query($con,$update_order_status);

      header("location:order_details.php?u_bill_no=".$_SESSION['billno']);
  }
            ...
            <div class="col-12 col-md-12 col-lg-12 order-2 order-md-1">
              <div class="row">
                <?php while($order_row = mysqli_fetch_assoc($cat_wise_data)) { 
                    ...
                    $select_order = "select * from product_order where user_id=$user_id and cat_id=$cat_id and order_status=0 and bill_no=$billno";
                  ...
                  <div class="info-box " style="<?php echo $back_color; ?>">
                    <div class="info-box-content">
                      ...
                      <span class="info-box-number text-center text-muted mb-0"><?php echo $order_row_data['sub_cat_name']  ?> : <?php echo $order_row_data['quantity']; ?></span>
                       ...
                       <a href="order_details.php?o_id=<?php echo $order_row['o_id']; ?>" class="stretched-link"></a>
                     <?php } ?>
                    </div>
                  </div>
                </div>
